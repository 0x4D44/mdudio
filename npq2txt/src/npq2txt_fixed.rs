...
fn main() -> io::Result<()> {
    ...
    writeln!(output_file, "Num Codebooks (K): {}", header.num_codebooks)?;
    writeln!(output_file, "Token Rate (fps): {}", header.token_rate)?;
    writeln!(output_file, "Original Bitrate (kbps): {}", header.orig_bitrate)?;
    writeln!(output_file, "Sequence Length (T): {}", header.seq_len)?;
    writeln!(output_file, "Vocab Sizes: {:?}", header.vocab_sizes)?;
    writeln!(output_file, "Dtype Code: {}", header.dtype_code)?;
    writeln!(output_file, "\nPayload:")?;

    // Write payload to output file
    for t in 0..header.seq_len as usize {
